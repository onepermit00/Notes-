...
10. **Shift Handoff + AI Summary polish (concierge)** — DAR home header now has visible "AI Summary" (ghost pill) and "End Shift" (coral pill) actions (data-testids: dar-ai-summary-btn, dar-end-shift-btn); previously these flows were only in DEAD CODE (renderShiftContent/renderNewTaskContent are defined but never rendered) + hidden command palette. handleClockOut now opens an editorial Shift Handoff drawer (dark recap chapter card: on-duty-since/tasks/open-requests, handoff-notes + open-items textareas, coral "End Shift & Send Handoff" via authApi.handoverShift(notes, items), "End shift without notes" skip). AI Summary modal + Shift Ended alert restyled editorially (dark band + coral, no red). CRITICAL LESSON: handover states MUST be declared before the Escape useEffect (TDZ crash "Cannot access showHandover before initialization" hit error boundary). Verified: end shift w/ notes → notes appear in next pre-shift briefing → clock back in.
