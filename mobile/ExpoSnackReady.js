// ADLTrack - Expo Snack Ready Version
// =====================================
// Copy this ENTIRE file to: https://snack.expo.dev
// Then add these dependencies in Snack:
...
const PATIENT = {
  ...
  emergencyContact: { name: 'Mary Smith', relationship: 'Daughter', phone: '(555) 234-5678' },
};
...
const TASKS = [
  ...
  { id: '7', title: 'Afternoon Medication', description: 'Administer afternoon meds', scheduledTime: '12:30 PM', priority: 'critical', status: 'pending', requiresPhoto: true },
  { id: '8', title: 'Rest Period', description: 'Assist with afternoon nap', scheduledTime: '2:00 PM', priority: 'quality', status: 'pending', requiresPhoto: false },
];

const VITALS = {
  bloodPressure: { systolic: 120, diastolic: 76, time: '10:30 AM' },
  heartRate: { value: 72, time: '10:30 AM' },
  temperature: { value: 98.6, time: '10:30 AM' },
  pain: { value: 2, time: '10:30 AM' },
};

const PROPOSED_TASKS = [
  { id: 'p1', title: 'Garden Walk', description: 'Short walk in the backyard', scheduledTime: '4:00 PM', proposedBy: 'Mary Smith (Daughter)' },
];

const INCIDENTS = [
  { id: 1, type: 'fall', severity: 'low', title: 'Minor Fall in Bathroom', description: 'Patient slipped while getting out of shower.', status: 'resolved', createdAt: '2025-01-15' },
];

const INCIDENT_TYPES = [
  { id: 'fall', label: 'Fall', icon: 'alert-circle', color: '#f43f5e' },
  { id: 'medication', label: 'Medication', icon: 'medical', color: '#f97316' },
  { id: 'behavioral', label: 'Behavioral', icon: 'brain', color: '#8b5cf6' },
  { id: 'injury', label: 'Injury', icon: 'bandage', color: '#ef4444' },
  { id: 'vital', label: 'Vital Concern', icon: 'thermometer', color: '#3b82f6' },
      ...
      <LinearGradient colors={['#ffffff', '#f0fdf4', '#dcfce7']} style={styles.gradient}>
        ...
        <View style={styles.hero}>
          ...
          <Text style={styles.heroDesc}>
            The caregiving app that brings families, caregivers, and agencies together.
          </Text>
        ...
        <View style={styles.featuresRow}>
          {[
            { icon: 'checkmark-circle', title: 'Tasks', desc: 'Track daily care' },
            { icon: 'heart', title: 'Vitals', desc: 'Health monitoring' },
            { icon: 'chatbubbles', title: 'Chat', desc: 'Stay connected' },
          ].map((f, i) => (
            <View key={i} style={styles.featureCard}>
              <View style={styles.featureIcon}>
                <Ionicons name={f.icon} size={22} color="#25D366" />
              </View>
        ...
        <View style={styles.trustBadge}>
          <Ionicons name="shield-checkmark" size={18} color="#25D366" />
...
const RoleSelectionScreen = ({ navigation }) => {
  ...
  const roles = [
    { id: 'caregiver', title: 'Caregiver', desc: 'Professional caregivers', icon: 'heart-circle', color: '#25D366', dark: true },
    { id: 'family', title: 'Family', desc: 'Family guardians', icon: 'people', color: '#3b82f6', dark: false },
    { id: 'agency', title: 'Agency', desc: 'Healthcare agencies', icon: 'business', color: '#8b5cf6', dark: false },
  ];
  ...
  return (
    <SafeAreaView style={styles.roleContainer}>
      ...
      <View style={styles.rolesContainer}>
        {roles.map((role) => (
          <TouchableOpacity key={role.id} style={styles.roleCard} onPress={() => selectRole(role)}>
...
const AuthScreen = ({ navigation, route }) => {
  ...
  const roleConfig = {
    ...
    family: { title: 'Family', color: '#3b82f6' },
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.toggleAuth}>
            <Text style={styles.toggleAuthText}>{isSignUp ? 'Have an account?' : 'No account?'}</Text>
            <TouchableOpacity onPress={() => setIsSignUp(!isSignUp)}>
              <Text style={[styles.toggleAuthLink, { color: config.color }]}>{isSignUp ? 'Sign In' : 'Sign Up'}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
    ...
    >
      ...
      <View style={styles.taskContent}>
        <View style={styles.taskMain}>
          <Text style={[styles.taskTitle, { color: theme.text }, isCompleted && styles.taskTitleCompleted]}>
            {task.title}
          </Text>
          <View style={styles.taskMeta}>
            <Ionicons name="time-outline" size={14} color={theme.textSecondary} />
            <Text style={[styles.taskTime, { color: theme.textSecondary }]}>{task.scheduledTime}</Text>
      ...
      {isExpanded && !isCompleted && (
        <View style={[styles.taskExpanded, { borderTopColor: theme.border, backgroundColor: theme.background }]}>
          ...
          <TouchableOpacity style={styles.startTaskBtn} onPress={() => onStart && onStart(task)}>
  ...
  <View style={[styles.requestCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
    ...
    <View style={styles.requestContent}>
      ...
      <View style={styles.requestActions}>
        <TouchableOpacity style={[styles.requestDeclineBtn, { borderColor: theme.border }]} onPress={() => onDecline && onDecline(task.id)}>
          <Text style={[styles.requestDeclineText, { color: theme.textSecondary }]}>Decline</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.requestAcceptBtn} onPress={() => onAccept && onAccept(task.id)}>
...
const HomeScreen = ({ navigation }) => {
  ...
  const handleCompleteTask = (note) => {
    if (selectedTask) {
      setTasks(tasks.map(t => t.id === selectedTask.id ? { ...t, status: 'completed', completionNote: note } : t));
  ...
  const handleAcceptRequest = (id) => {
    ...
    if (request) {
      setTasks([...tasks, { ...request, status: 'pending', priority: 'quality' }]);
        ...
        <View style={styles.offlineContainer}>
          ...
          <View style={styles.startShiftSection}>
            ...
            <TouchableOpacity style={styles.goButton} onPress={startShift}>
              ...
              <LinearGradient colors={['#f43f5e', '#e11d48']} style={styles.goButtonInner}>
  ...
  return (
    <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.vitalsContainer}>
          <View style={styles.vitalsGrid}>
            <TouchableOpacity style={[styles.vitalCard, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setShowVitals(true)}>
              <View style={[styles.vitalIcon, { backgroundColor: 'rgba(244,63,94,0.2)' }]}>
                <Ionicons name="heart" size={18} color="#f43f5e" />
              </View>
              <Text style={[styles.vitalValue, { color: theme.text }]}>{VITALS.bloodPressure.systolic}/{VITALS.bloodPressure.diastolic}</Text>
            ...
            <TouchableOpacity style={[styles.vitalCard, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setShowVitals(true)}>
              ...
              <Text style={[styles.vitalValue, { color: theme.text }]}>{VITALS.heartRate.value}</Text>
            ...
            <TouchableOpacity style={[styles.vitalCard, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setShowVitals(true)}>
              ...
              <Text style={[styles.vitalValue, { color: theme.text }]}>{VITALS.temperature.value}°</Text>
            ...
            <TouchableOpacity style={[styles.vitalCard, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => setShowVitals(true)}>
              ...
              <Text style={[styles.vitalValue, { color: theme.text }]}>{VITALS.pain.value}/10</Text>
        ...
        <View style={styles.taskTabs}>
          <TouchableOpacity onPress={() => setActiveTab('requests')}>
            <Text style={[styles.tabText, { color: activeTab === 'requests' ? theme.text : theme.textMuted }]}>
              Requests {proposedTasks.length > 0 && `(${proposedTasks.length})`}
            </Text>
          </TouchableOpacity>
          <View style={[styles.tabDivider, { backgroundColor: theme.border }]} />
          <TouchableOpacity onPress={() => setActiveTab('tasks')}>
            <Text style={[styles.tabText, { color: activeTab === 'tasks' ? theme.text : theme.textMuted }]}>Today's Tasks</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.taskList}>
          {activeTab === 'tasks' ? (
            tasks.map((task) => (
              <TaskCard key={task.id} task={task} theme={theme} isExpanded={expandedTaskId === task.id} onToggle={toggleTask} onStart={handleStartTask} />
            ))
          ) : proposedTasks.length > 0 ? (
            proposedTasks.map((task) => (
              <TaskRequestCard key={task.id} task={task} theme={theme} onAccept={handleAcceptRequest} onDecline={handleDeclineRequest} />
            ))
          ) : (
            <View style={[styles.emptyState, { backgroundColor: theme.card, borderColor: theme.border }]}>
              <Ionicons name="sparkles" size={36} color={theme.textMuted} />
      ...
      {/* AI FAB */}
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AICopilot')}>
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={[styles.modalContainer, { backgroundColor: theme.background }]}>
        ...
        <ScrollView style={styles.modalContent}>
          ...
          <TouchableOpacity style={styles.completeBtn} onPress={() => onComplete(note)}>
  ...
  <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
    <SafeAreaView style={[styles.modalContainer, { backgroundColor: theme.background }]}>
      ...
      <ScrollView style={styles.modalContent}>
          ...
          <View style={[styles.profileSectionCard, { backgroundColor: theme.card }]}>
            {PATIENT.conditions.map((c, i) => (
              <View key={i} style={[styles.profileItem, i < PATIENT.conditions.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
                ...
                <Text style={[styles.profileItemText, { color: theme.text }]}>{c}</Text>
              </View>
            ))}
          </View>
        </View>
          ...
          <View style={[styles.profileSectionCard, { backgroundColor: theme.card }]}>
            {PATIENT.allergies.map((a, i) => (
              <View key={i} style={[styles.profileItem, i < PATIENT.allergies.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
                ...
                <Text style={[styles.profileItemText, { color: theme.text }]}>{a}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={styles.profileSection}>
          <Text style={[styles.profileSectionTitle, { color: theme.text }]}>Medications</Text>
          <View style={[styles.profileSectionCard, { backgroundColor: theme.card }]}>
            {PATIENT.medications.map((m, i) => (
              <View key={i} style={[styles.profileItem, styles.profileItemColumn, i < PATIENT.medications.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
                <View style={styles.profileItemHeader}>
                  <Ionicons name="medkit" size={16} color={theme.blue} />
                  <Text style={[styles.profileItemText, { color: theme.text }]}>{m.name}</Text>
                </View>
                <Text style={[styles.profileItemSub, { color: theme.textSecondary }]}>{m.dosage} - {m.frequency}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={styles.profileSection}>
          ...
          <View style={[styles.emergencyCard, { backgroundColor: `${theme.danger}15` }]}>
            <View style={styles.emergencyHeader}>
              ...
              <Text style={[styles.emergencyName, { color: theme.text }]}>{PATIENT.emergencyContact.name}</Text>
            </View>
            <Text style={[styles.emergencyRelation, { color: theme.textSecondary }]}>{PATIENT.emergencyContact.relationship}</Text>
...
const VitalsModal = ({ visible, onClose, theme }) => {
  ...
  const getVitalInfo = () => {
    switch (activeVital) {
      case 'bp': return { value: `${VITALS.bloodPressure.systolic}/${VITALS.bloodPressure.diastolic}`, unit: 'mmHg' };
      case 'hr': return { value: VITALS.heartRate.value, unit: 'bpm' };
      case 'temp': return { value: VITALS.temperature.value, unit: '°F' };
      case 'pain': return { value: VITALS.pain.value, unit: '/10' };
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={[styles.modalContainer, { backgroundColor: theme.background }]}>
        <View style={[styles.modalHeader, { borderBottomColor: theme.border }]}>
          ...
          <Text style={[styles.modalTitle, { color: theme.text }]}>Vitals</Text>
          <View style={{ width: 26 }} />
        </View>

        <View style={[styles.vitalTabs2, { borderBottomColor: theme.border }]}>
          {vitalTabs.map((tab) => (
            <TouchableOpacity
              key={tab.id}
              style={[styles.vitalTab, activeVital === tab.id && { backgroundColor: `${tab.color}20` }]}
              onPress={() => setActiveVital(tab.id)}
            >
              <Ionicons name={tab.icon} size={18} color={activeVital === tab.id ? tab.color : theme.textMuted} />
              <Text style={[styles.vitalTabText, { color: activeVital === tab.id ? tab.color : theme.textMuted }]}>{tab.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <ScrollView style={styles.modalContent}>
          <View style={[styles.vitalCurrentCard, { backgroundColor: theme.card }]}>
            ...
            <Text style={[styles.vitalCurrentValue, { color: theme.text }]}>
              {getVitalInfo().value}
              <Text style={[styles.vitalCurrentUnit, { color: theme.textSecondary }]}> {getVitalInfo().unit}</Text>
            </Text>
          </View>
      ...
      <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
        <View style={styles.successContainer}>
          <View style={[styles.successCard, { backgroundColor: theme.card }]}>
            ...
            <Text style={[styles.successDesc, { color: theme.textSecondary }]}>Incident documented successfully.</Text>
      ...
      <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
        <View style={[styles.incidentFormHeader, { borderBottomColor: theme.border }]}>
          <View style={styles.incidentFormHeaderLeft}>
            ...
            <View>
              <Text style={[styles.incidentFormTitle, { color: theme.text }]}>New Incident</Text>
              <Text style={[styles.incidentFormStep, { color: theme.textMuted }]}>Step {step} of 3</Text>
            </View>
          </View>
          <TouchableOpacity style={[styles.cancelBtn, { backgroundColor: theme.cardHover }]} onPress={() => setShowNewIncident(false)}>
        ...
        <View style={styles.stepProgress}>
          {[1, 2, 3].map((s) => (
            <View key={s} style={[styles.stepDot, { backgroundColor: s <= step ? '#f43f5e' : theme.border }]} />
          ))}
        </View>
        ...
        <ScrollView style={styles.incidentFormContent}>
          {step === 1 && (
            <>
              <Text style={[styles.stepTitle, { color: theme.text }]}>Incident Type</Text>
              <View style={styles.typeGrid}>
                {INCIDENT_TYPES.map((type) => (
                  <TouchableOpacity
                    key={type.id}
                    style={[styles.typeCard, { backgroundColor: theme.card, borderColor: incidentType === type.id ? type.color : theme.border }, incidentType === type.id && { borderWidth: 2 }]}
          ...
          {step === 2 && (
            <>
              <Text style={[styles.stepTitle, { color: theme.text }]}>Severity</Text>
              <View style={styles.severityList}>
                {SEVERITY_LEVELS.map((sev) => (
                  <TouchableOpacity
                    key={sev.id}
                    style={[styles.severityCard, { backgroundColor: theme.card, borderColor: severity === sev.id ? sev.color : theme.border }, severity === sev.id && { borderWidth: 2 }]}
          ...
          {step === 3 && (
            <>
              <Text style={[styles.stepTitle, { color: theme.text }]}>Description</Text>
              <TextInput
  ...
  return (
    <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.incidentScroll}>
        ...
        <TouchableOpacity style={styles.newIncidentBtn} onPress={() => setShowNewIncident(true)}>
          <Ionicons name="add-circle" size={24} color="#fff" />
          <View style={{ marginLeft: 12 }}>
            <Text style={styles.newIncidentTitle}>New Incident Report</Text>
            <Text style={styles.newIncidentSubtitle}>Document an incident</Text>
          </View>
        ...
        {incidents.map((inc) => (
          <View key={inc.id} style={[styles.incidentCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <View style={[styles.incidentStripe, { backgroundColor: inc.status === 'open' ? '#f43f5e' : '#25D366' }]} />
            <View style={styles.incidentContent}>
              <Text style={[styles.incidentTitle, { color: theme.text }]}>{inc.title}</Text>
              <Text style={[styles.incidentDesc, { color: theme.textSecondary }]}>{inc.description}</Text>
              <Text style={[styles.incidentDate, { color: theme.textMuted }]}>{inc.createdAt}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
...
const MessageScreen = () => {
  const { theme } = useTheme();
  const conversations = [
    { id: 1, name: 'Mary Smith', role: 'Daughter', lastMessage: 'Thank you for the update!', time: '2m ago', unread: 2, color: '#3b82f6' },
    { id: 2, name: 'Dr. Johnson', role: 'Physician', lastMessage: 'BP looks good.', time: '1h ago', unread: 0, color: '#8b5cf6' },
    { id: 3, name: 'Care Agency', role: 'Agency', lastMessage: 'Schedule confirmed.', time: '3h ago', unread: 0, color: '#f97316' },
  ];
    ...
    <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.messageScroll}>
        ...
        {conversations.map((conv) => (
          <TouchableOpacity key={conv.id} style={[styles.conversationCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <View style={[styles.avatar, { backgroundColor: conv.color }]}>
              <Text style={styles.avatarText}>{conv.name.split(' ').map(n => n[0]).join('')}</Text>
            </View>
            <View style={styles.conversationInfo}>
              <View style={styles.conversationHeader}>
                <Text style={[styles.conversationName, { color: theme.text }]}>{conv.name}</Text>
                <Text style={[styles.conversationTime, { color: theme.textMuted }]}>{conv.time}</Text>
              </View>
              <Text style={[styles.conversationRole, { color: theme.textSecondary }]}>{conv.role}</Text>
              <Text style={[styles.conversationMessage, { color: theme.textSecondary }]} numberOfLines={1}>{conv.lastMessage}</Text>
            </View>
            {conv.unread > 0 && (
    ...
    <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.shiftScroll}>
        ...
        <View style={[styles.shiftCard, { backgroundColor: isShiftActive ? '#25D366' : theme.card, borderColor: isShiftActive ? '#25D366' : theme.border }]}>
          <View style={styles.shiftCardHeader}>
            <View style={styles.shiftStatus}>
              <View style={[styles.statusDot, { backgroundColor: isShiftActive ? '#fff' : theme.textMuted }]} />
              <Text style={[styles.shiftStatusText, { color: isShiftActive ? '#fff' : theme.text }]}>
            ...
            <View style={styles.shiftDetails}>
              <View style={styles.shiftDetailRow}>
                ...
                <View style={styles.shiftDetailText}>
                  <Text style={styles.shiftDetailLabel}>Started</Text>
                  <Text style={styles.shiftDetailValue}>8:00 AM</Text>
                </View>
              </View>
              <View style={styles.shiftDetailRow}>
          ...
          <TouchableOpacity
            style={[styles.shiftToggleBtn, { backgroundColor: isShiftActive ? 'rgba(255,255,255,0.2)' : '#25D366' }]}
            onPress={() => setIsShiftActive(!isShiftActive)}
          >
            <Ionicons name={isShiftActive ? 'stop' : 'play'} size={20} color="#fff" />
            <Text style={styles.shiftToggleBtnText}>{isShiftActive ? 'End Shift' : 'Start Shift'}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={[styles.earningsCard, { backgroundColor: theme.card }]}>
          ...
          <Text style={[styles.earningsAmount, { color: theme.text }]}>$1,240</Text>
          <Text style={[styles.earningsSubtext, { color: theme.textSecondary }]}>32 hours worked</Text>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const MenuScreen = ({ navigation }) => {
  ...
  const menuItems = [
    ...
    { label: 'Shift History', icon: 'time-outline' },
  ...
  return (
    <SafeAreaView style={[styles.homeContainer, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.menuScroll}>
        <View style={[styles.profileCardMenu, { backgroundColor: theme.card }]}>
        ...
        <View style={[styles.themeToggle, { backgroundColor: theme.card }]}>
          <View style={styles.themeToggleLeft}>
            <View style={[styles.themeIcon, { backgroundColor: isDark ? '#8b5cf620' : '#f59e0b20' }]}>
            ...
            <View>
              ...
              <Text style={[styles.themeValue, { color: theme.textSecondary }]}>{isDark ? 'Dark' : 'Light'}</Text>
            </View>
          </View>
          <Switch
            value={isDark}
        ...
        <View style={[styles.menuContainer, { backgroundColor: theme.card }]}>
          {menuItems.map((item, idx) => (
            <TouchableOpacity
              key={idx}
              style={[styles.menuItem, idx < menuItems.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}
            >
              <View style={styles.menuItemLeft}>
                <View style={[styles.menuIcon, { backgroundColor: theme.cardHover }]}>
        ...
        <TouchableOpacity style={[styles.signOutBtn, { backgroundColor: `${theme.danger}15` }]} onPress={logout}>
...
const AICopilotScreen = ({ navigation }) => {
  ...
  const [messages, setMessages] = useState([
    { id: 1, type: 'ai', text: "Hello Julia! I'm your AI care assistant. How can I help you today?" },
  ]);
  ...
  const sendMessage = async (text) => {
    ...
    setTimeout(() => {
      const aiMsg = { id: Date.now() + 1, type: 'ai', text: `I can help with "${text}". For David Smith's care today, I recommend following the scheduled tasks and monitoring his vitals regularly.` };
  ...
  return (
    <SafeAreaView style={[styles.copilotContainer, { backgroundColor: theme.background }]}>
      <View style={[styles.copilotHeader, { borderBottomColor: theme.border }]}>
        <TouchableOpacity style={[styles.copilotBackBtn, { backgroundColor: theme.cardHover }]} onPress={() => navigation.goBack()}>
        ...
        <View style={styles.copilotHeaderTitle}>
          ...
          <View>
            <Text style={[styles.copilotTitle, { color: theme.text }]}>AI Copilot</Text>
            <Text style={[styles.copilotSubtitle, { color: theme.textSecondary }]}>Care Assistant</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView style={styles.messagesList}>
        {messages.map((msg) => (
          <View key={msg.id} style={[styles.messageBubble, msg.type === 'user' ? [styles.userBubble, { backgroundColor: theme.primary }] : [styles.aiBubble, { backgroundColor: theme.card }]]}>
            {msg.type === 'ai' && (
              <View style={styles.aiAvatarSmall}>
                <Ionicons name="sparkles" size={12} color="#fff" />
              </View>
            )}
            <Text style={[styles.messageText, { color: msg.type === 'user' ? '#fff' : theme.text }]}>{msg.text}</Text>
          </View>
        ))}
        {loading && (
      ...
      <View style={styles.suggestionsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.suggestionsList}>
          {suggestions.map((sug, idx) => (
            <TouchableOpacity key={idx} style={[styles.suggestionChip, { backgroundColor: theme.card, borderColor: theme.border }]} onPress={() => sendMessage(sug)}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.inputBar, { borderTopColor: theme.border, backgroundColor: theme.background }]}>
          <TextInput
            style={[styles.chatInput, { backgroundColor: theme.card, color: theme.text }]}
            ...
            multiline
          />
          <TouchableOpacity style={[styles.sendBtn, { backgroundColor: input.trim() ? theme.primary : theme.cardHover }]} onPress={() => sendMessage(input)} disabled={!input.trim()}>
...
const CaregiverTabs = () => {
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        headerShown: false,
        tabBarStyle: { backgroundColor: theme.card, borderTopColor: theme.border, borderTopWidth: 1, height: 85, paddingTop: 8, paddingBottom: 28 },
        tabBarActiveTintColor: route.name === 'Incident' ? theme.danger : theme.primary,
        tabBarInactiveTintColor: theme.textMuted,
        tabBarLabelStyle: { fontSize: 11, fontWeight: '600', marginTop: 4 },
        tabBarIcon: ({ focused, color }) => {
          const icons = { Home: focused ? 'home' : 'home-outline', Incident: focused ? 'alert-circle' : 'alert-circle-outline', Message: focused ? 'chatbubble' : 'chatbubble-outline', Shifts: focused ? 'time' : 'time-outline', Menu: focused ? 'menu' : 'menu-outline' };
...
const styles = StyleSheet.create({
  ...
  ctaBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#25D366', paddingVertical: 16, paddingHorizontal: 32, borderRadius: 16, gap: 10 },
  ctaBtnText: { fontSize: 17, fontWeight: '600', color: '#fff' },
  featuresRow: { flexDirection: 'row', gap: 12, marginBottom: 32 },
  featureCard: { flex: 1, backgroundColor: '#fff', borderRadius: 16, padding: 16, alignItems: 'center' },
  ...
  featureTitle: { fontSize: 13, fontWeight: '600', color: '#1c1917', marginBottom: 4 },
  featureDesc: { fontSize: 11, color: '#78716c', textAlign: 'center' },
  trustBadge: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingBottom: 24 },
  trustText: { fontSize: 12, fontWeight: '600', color: '#25D366', letterSpacing: 0.5 },
  ...
  roleSubtitle: { fontSize: 16, color: '#78716c', marginBottom: 32 },
  rolesContainer: { gap: 16 },
  roleCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderWidth: 1, borderColor: '#e7e5e4', borderRadius: 20, padding: 20 },
  ...
  socialBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', borderWidth: 1, borderColor: '#e7e5e4', borderRadius: 14, paddingVertical: 14, gap: 12, marginBottom: 12 },
  socialBtnText: { fontSize: 16, fontWeight: '500', color: '#1c1917' },
  ...
  toggleAuth: { flexDirection: 'row', justifyContent: 'center', gap: 6, marginTop: 24 },
  toggleAuthText: { fontSize: 15, color: '#78716c' },
  toggleAuthLink: { fontSize: 15, fontWeight: '600' },

  // Home - Offline
  offlineContainer: { flex: 1, paddingHorizontal: 24 },
  offlineHeader: { flexDirection: 'row', alignItems: 'center', gap: 14, paddingTop: 16, paddingBottom: 24 },
  ...
  vitalCard: { flex: 1, borderRadius: 16, padding: 12, borderWidth: 1, alignItems: 'center' },
  ...
  photoBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, gap: 4 },
  ...
  requestDeclineBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, borderWidth: 1, alignItems: 'center' },
  requestDeclineText: { fontSize: 15, fontWeight: '500' },
  requestAcceptBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, backgroundColor: '#25D366', alignItems: 'center' },
  ...
  noteInput: { height: 100, borderRadius: 16, borderWidth: 1, padding: 14, fontSize: 15 },
  completeBtn: { flexDirection: 'row', backgroundColor: '#25D366', paddingVertical: 16, borderRadius: 14, justifyContent: 'center', alignItems: 'center', gap: 10 },
  ...
  profileItem: { flexDirection: 'row', alignItems: 'center', padding: 14, gap: 12 },
  ...
  profileItemSub: { fontSize: 13, marginTop: 4, marginLeft: 28 },
  ...
  callButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, borderRadius: 12, gap: 8 },
  callButtonText: { fontSize: 15, fontWeight: '600', color: '#fff' },

  // Vitals Modal
  vitalTabs2: { flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, gap: 8 },
  vitalTab: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 12, gap: 6 },
  ...
  recordVitalBtn: { flexDirection: 'row', backgroundColor: '#25D366', paddingVertical: 14, borderRadius: 14, justifyContent: 'center', alignItems: 'center', gap: 8 },
  recordVitalBtnText: { fontSize: 16, fontWeight: '600', color: '#fff' },
  ...
  incidentFormIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: '#f43f5e15', justifyContent: 'center', alignItems: 'center' },
  ...
  stepProgress: { flexDirection: 'row', justifyContent: 'center', gap: 8, paddingVertical: 16 },
  stepDot: { width: 8, height: 8, borderRadius: 4 },
  incidentFormContent: { flex: 1, padding: 20 },
  stepTitle: { fontSize: 20, fontWeight: '700', marginBottom: 20 },
  typeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 12 },
  typeCard: { width: (width - 64) / 2, borderRadius: 16, padding: 16, borderWidth: 1, alignItems: 'center' },
  ...
  severityList: { gap: 12 },
  severityCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, padding: 16, borderWidth: 1, gap: 12 },
  severityDot: { width: 12, height: 12, borderRadius: 6 },
  severityLabel: { fontSize: 16, fontWeight: '500' },
  textArea: { height: 120, borderRadius: 14, borderWidth: 1, padding: 14, fontSize: 15 },
  stepNav: { flexDirection: 'row', gap: 12, marginTop: 24 },
  backStepBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, borderWidth: 1, alignItems: 'center' },
  backStepBtnText: { fontSize: 15, fontWeight: '500' },
  nextBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: '#f43f5e', alignItems: 'center' },
  nextBtnText: { fontSize: 15, fontWeight: '600', color: '#fff' },
  submitBtn: { flex: 1, paddingVertical: 14, borderRadius: 12, backgroundColor: '#25D366', alignItems: 'center' },
  ...
  shiftDetailRow: { width: '50%', flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 },
  shiftDetailText: {},
  shiftDetailLabel: { fontSize: 11, color: 'rgba(255,255,255,0.7)' },
  shiftDetailValue: { fontSize: 14, fontWeight: '600', color: '#fff' },
  shiftToggleBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 14, borderRadius: 12, gap: 10 },
  shiftToggleBtnText: { fontSize: 16, fontWeight: '600', color: '#fff' },
  ...
  profileCardMenu: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 18 },
  ...
  themeToggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14, borderRadius: 14, marginBottom: 16 },
  ...
  signOutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 14, borderRadius: 14, gap: 10 },
  ...
  copilotHeaderTitle: { flex: 1, flexDirection: 'row', alignItems: 'center', marginLeft: 8, gap: 10 },
  ...
  suggestionsList: { paddingHorizontal: 14, gap: 8 },
  ...
  chatInput: { flex: 1, minHeight: 40, maxHeight: 100, borderRadius: 20, paddingHorizontal: 16, paddingVertical: 10, fontSize: 15 },
