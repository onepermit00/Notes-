...
export const TaskRequestCard = ({ task, onAccept, onDecline }) => {
  ...
  return (
    <div style={{ background: BG, border: '1.5px solid rgba(255,56,92,0.22)', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 16px rgba(255,56,92,0.06)', fontFamily: INTER }}>
      <div style={{ padding: 20 }}>

        {/* Header: icon tile + badge + title */}
        ...
        {showDeclineForm ? (
          <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setShowDeclineForm(false)}
                style={{ flex: 1, padding: '12px 0', background: BG, border: `1px solid ${BORDER}`, borderRadius: 12, fontFamily: INTER, fontSize: 13, fontWeight: 600, color: MUTED, cursor: 'pointer' }}>
                Cancel
              </button>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => setShowDeclineForm(true)} data-testid={`decline-request-${task.id}`}
              style={{ flex: 1, padding: '13px 0', background: BG, border: `1px solid ${BORDER}`, borderRadius: 12, fontFamily: INTER, fontSize: 14, fontWeight: 700, color: MUTED, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
              <X size={16} /> Decline
            </button>
          </div>
        )}
