      ...
      <AnimatePresence>
        ...
        {taskOpen && (
          <motion.div key="task-modal" ref={taskModalRef}
            ...
            initial={{ x:'110%' }} animate={{ x:0 }} exit={{ x:'110%' }}
            transition={{ type:'spring', damping:32, stiffness:300 }}
            style={{ position:'fixed', right:16, top:16, bottom:16, width:Math.min(isMobile ? 700 : 580, window.innerWidth - (isMobile ? 32 : 280)), zIndex:68, background:BG, borderRadius:24, boxShadow:'0 24px 64px rgba(0,0,0,0.20)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        {conOpen && (
          <motion.div key="con-panel"
            initial={{ x: '110%' }} animate={{ x: 0 }} exit={{ x: '110%' }}
            transition={{ type: 'spring', damping: 32, stiffness: 300 }}
            style={{ position: 'fixed', right: 16, top: 16, bottom: 16, width: Math.min(isMobile ? 700 : 580, window.innerWidth - (isMobile ? 32 : 280)), background: BG, zIndex: 68, display: 'flex', flexDirection: 'column', borderRadius: 24, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.20)' }}>
        ...
        {profileOpen && (
          <>
            ...
            <motion.div key="prof-panel"
              initial={{ x:'110%' }} animate={{ x:0 }} exit={{ x:'110%' }}
              transition={{ type:'spring', damping:32, stiffness:300 }}
              style={{ position:'fixed', right:16, top:16, bottom:16, width:Math.min(isMobile ? 700 : 580, window.innerWidth - (isMobile ? 32 : 280)), zIndex:70, background:BG, borderRadius:24, boxShadow:'0 24px 64px rgba(0,0,0,0.20)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
              {/* Header */}
